...
- [x] 3.6 Implement hierarchical sorting for Raids: Boss Header > Items. (Added: Unique keys for headers to fix anchor dependency errors).
...
- [x] 4.3 Ensure the migration function safely handles existing items that lack `encounterID` and `instanceID` by scanning the EJ API to fetch and populate this missing information.
